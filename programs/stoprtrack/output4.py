    ...
    for obj_id, points in object_trajectories.items():
        ...
        if len(visible_points) > 1:
            for i in range(1, len(visible_points)):
                if frame_count < 130:  # ✅ Only draw green trajectory before frame 110
                    cv2.line(frame, (visible_points[i - 1][1], visible_points[i - 1][2]),
                             (visible_points[i][1], visible_points[i][2]), (0, 255, 0), 3)

                    cv2.line(image_with_trajectory, (visible_points[i - 1][1], visible_points[i - 1][2]),
                             (visible_points[i][1], visible_points[i][2]), (0, 255, 0), 3)

            # ✅ Draw red ball only if frame count < 110
            if frame_count < 130:
                cv2.circle(frame, (visible_points[-1][1], visible_points[-1][2]), 7, (0, 0, 255), -1)
                cv2.circle(image_with_trajectory, (visible_points[-1][1], visible_points[-1][2]), 7, (0, 0, 255), -1)
